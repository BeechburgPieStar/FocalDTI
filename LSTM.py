...
from keras.layers import Dense, Dropout, LSTM, regularizers
...
test = pd.read_csv(os.path.abspath('data/test2.csv'), usecols=use_cols, low_memory=False)
...
dr = 0.5
model = Sequential()
model.add(LSTM(256, input_dim=297, input_length=1, return_sequences=True, activation='relu', recurrent_activation='hard_sigmoid'))
model.add(Dropout(dr))
model.add(LSTM(128, activation='relu', recurrent_activation='hard_sigmoid', return_sequences=False))
model.add(Dropout(dr))
# model.add(LSTM(64, activation='relu', recurrent_activation='hard_sigmoid', return_sequences=False))
# model.add(Dropout(dr))
...
#               optimizer='adam',
#               metrics=['accuracy'])
model.compile(loss=[catergorical_focal_loss()], metrics=["accuracy"], optimizer=sgd)
model_path ="/data/hzm/IoT-device-type-identification-master/deep_learning/LSTM/multiepoch"
...
tensorboard = TensorBoard("/data/hzm/IoT-device-type-identification-master/deep_learning/LSTM/multiepoch/log", 0)
...
model.save("/data/hzm/IoT-device-type-identification-master/deep_learning/LSTM/multiepoch/lstm_100.hdf5")
